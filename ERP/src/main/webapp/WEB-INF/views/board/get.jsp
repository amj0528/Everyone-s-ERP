				...
				<div align="right">
					...
					<sec:authorize access="isAuthenticated()">
						<c:if test="${pinfo.username eq vo.writer}">
							<button style='color:white;' class="btn btn-warning" id="boardModBtn">수정</button>
						</c:if>
		...
		<div class="panel-body">
			<div class="form-group row">
				...
				<div class="col-1" style="padding-top:5px;">
					<button id="btnreply" class="reply"
						style="color: white; width:80px; height:40px; border:none; border-radius:5px; background-color: #343A40;" class="btn">저장</button>
				</div>
...
<script>
	...
	var answer = "${v.answer}"; // 답변여부
	var declar = "${v.declar}"; // 신고여부 
			...
			.ready(
					function() {
						...
						if (reply == "false") {
							$("#replyview")
									.html(
											'<br><b>※댓글 기능을 지원하지 않는 게시판입니다.<b/><br><br>');
						}
						ReplyList();
						FileList();
						$('#boardModBtn')
								.on(
										...
										function(e) {
											e.preventDefault();
											window.location.href = "/board/update?code="
													+ $("#code").val()
													+ "&num=" + $("#num").val();
						...
						$('#boardAnswerBtn')
								.on(
										...
										function(e) {
											e.preventDefault();
											window.location.href = "/board/answer?code="
													+ $("#code").val()
													+ "&num=" + $("#num").val();
										});
						...
						$('#boardDeclarBtn')
								.on(
										...
										function(e) {
											e.preventDefault();
											window.location.href = "/board/declar?code="
													+ $("#code").val()
													+ "&num=" + $("#num").val();
										});
						$(document).on(
